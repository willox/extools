...
bool Core::find_functions()
{
	...
	string_table = *(StringTable**)Pocket::Sigscan::FindPattern(BYONDCORE, "A1 ?? ?? ?? ?? 8B 04 ?? 85 C0 0F 85 ?? ?? ?? ?? 83 3D ?? ?? ?? ?? 00 0F 85 ?? ?? ?? ??", 1);

#else
	string_table = *(StringTable**)Pocket::Sigscan::FindPattern(BYONDCORE, "55 89 e5 83 ec ?? 8b 45 ?? 39 05 ?? ?? ?? ?? 76 ?? 8b 15 ?? ?? ?? ?? 8b 04 ?? 85 c0", 19);
